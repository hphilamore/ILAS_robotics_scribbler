...
static int ping_dist_cm[n_servo_pos]; //= {0, 0, 0, 0, 0, 0, 0, 0, 0};   // an array to hold encoder data
...
void obstacle_avoid_ping_array(void){
  ...
  print("%d,", ping_dist_cm[smallest]);
  
  for(int i=1; i < n_servo_pos; i++){
    
    print("%d,", ping_dist_cm[i]);
    
    if(ping_dist_cm[i] < ping_dist_cm[smallest]){
			smallest = i;
    }
  } 
  print("]");
  
  print("\t %d \t", servo_pos_array[smallest]);
  
  

   
  if(ping_dist_cm[smallest] < th_ping_avoid_cm){
    //s3_setLED(S3_LEFT, S3_COLOR_FF0000 );     s3_setLED(S3_CENTER, S3_COLOR_FF0000 );     s3_setLED(S3_RIGHT, S3_COLOR_FF0000); 
    print(" %d \t", ping_dist_cm[smallest]);

    if(servo_pos_array[smallest] < 90){
      // obstacle to right of robot
    ...
    else if(servo_pos_array[smallest] > 90){
      // obstacle to left of robot
