...
int SenseLineRight;
int SenseLineLeft;
int turnFromLineComplete_dist = bot_diameter * PI / 4;
int leftave_white; 
int rightave_white; 
int leftave_black; 
int rightave_black; 
...
void sense(void){
 ...
 if (s3_lineSensor(S3_LEFT) < mid_left) {
  ...
  TurnFromLineComplete = fabs(encoder_vals[0] - line_left_count_start) > TurnFromLineComplete_dist; //50 ;
...
{    
  ...
  */
  ...
  if( SenseLineLeft || SenseLineRight ){
    readout = "Turning from Line"; 
    //resetTurnFromLine();
    left_speed = basic_speed;
    right_speed = -basic_speed;
