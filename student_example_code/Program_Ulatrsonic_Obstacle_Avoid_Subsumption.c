...
void sense(void){
  ...
  */
  ...
  //print("%d \t %d \t", servo_pos, servo_index);
  ...
  //servo_change_dir = servo_index >= (n_servo_pos - 1) || servo_index <= 0;
...
{    
  ...
  */
  ...
  if( obstacle * closest_is_right ){
    // obstacle to right of robot, turn left
    left_speed = -20;    
    right_speed = 70;
  ...
  if( obstacle * closest_is_left ){
    // obstacle to left of robot, turn right
  ...
  if( obstacle * closest_is_front ){
    AvoidFrontObstacle = 1;
    //center_colour = S3_CENTER, S3_COLOR_FF0000;
    //readout = "Obstacle Detected, Obstacle is Front"; 
  ...
  //print("%d,", servo_pos_array[min_ping_index]);
...
void obstacle_avoid_ping_array(void){
  ...
  if(ping_dist_cm[min_index] < th_ping_avoid_cm){
    //s3_setLED(S3_LEFT, S3_COLOR_FF0000 );     s3_setLED(S3_CENTER, S3_COLOR_FF0000 );     s3_setLED(S3_RIGHT, S3_COLOR_FF0000); 
    print(" %d \t", ping_dist_cm[min_index]);

    if(servo_pos_array[min_index] < 90){
      // obstacle to right of robot
      s3_setLED( S3_RIGHT, S3_COLOR_FF0000 ); 
    ...
    else if(servo_pos_array[min_index] > 90){
      // obstacle to left of robot
      s3_setLED( S3_LEFT, S3_COLOR_FF0000 );
      ...
      // obstacle is in front of ro
      //s3_motorSet(-100, 100, 0);  // turn fast on spot
